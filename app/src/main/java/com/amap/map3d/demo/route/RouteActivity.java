...
import com.amap.map3d.demo.poisearch.ToastUtil;
...
public class RouteActivity extends Activity implements OnMapClickListener,
	...
	// 116.397743);//起点，
	private LatLonPoint mEndPoint;// = new LatLonPoint(39.984947,
	// 116.494689);//终点，
	...
	protected void onCreate(Bundle bundle) {
		...
		String endlatitude = getIntent().getStringExtra("endlatitude");
		String endlongitude = getIntent().getStringExtra("endlongitude");
		String startlatitude = getIntent().getStringExtra("startlatitude");
		String startlongitude = getIntent().getStringExtra("startlongitude");
		double endlatitudes = Double.parseDouble(endlatitude);
		double endlongitudes = Double.parseDouble(endlongitude);
		double startlatitudes = Double.parseDouble(startlatitude);
		double startlongitudes = Double.parseDouble(startlongitude);
		mStartPoint = new LatLonPoint(startlatitudes, startlongitudes);
		mEndPoint = new LatLonPoint(endlatitudes, endlongitudes);
		Config.start_nav=new NaviLatLng(startlatitudes, startlongitudes);
		Config.end_nav=new NaviLatLng(endlatitudes, endlongitudes);
					...
					.addMarker(new MarkerOptions()
							...
							.icon(BitmapDescriptorFactory
									.fromBitmap(BitmapFactory.decodeResource(
											getResources(), R.drawable.tips_1)))
							.position(
									new LatLng(mStartPoint.getLatitude(),
											mStartPoint.getLongitude())));
			locationMarker.showInfoWindow();
			aMap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(
					mStartPoint.getLatitude(), mStartPoint.getLongitude()), 14));
					*/
